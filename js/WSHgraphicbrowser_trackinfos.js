...
var properties = {
	...
	showInLibrary_RightPlaylistOn: window.GetProperty(
		"MAINPANEL adapt now playing to left menu righ playlist on",
		true
	),
	showInLibrary_RightPlaylistOff: window.GetProperty(
...
properties.smooth_scroll_value =
	properties.smooth_scroll_value < 0
		? 0
		: properties.smooth_scroll_value > 0.9
		? 0.9
		: properties.smooth_scroll_value;
properties.smooth_expand_value =
	properties.smooth_expand_value < 0
		? 0
		: properties.smooth_expand_value > 0.9
		? 0.9
		: properties.smooth_expand_value;
...
var TF = {
	...
	titlePCB: fb.TitleFormat(
		properties.TFtitle + " ^^ " + properties.TFplaycount + " - " + properties.TFcodec + " - " + properties.TFbitrate
	),
...
oPlaylistHistory = function () {
	if (globalProperties.logFns_oPlaylistHistory) {
		console.log("WSH_GBrowser: called oPlaylistHistory ( )");
	}
	this.history = Array();
	this.trace = function () {
		if (globalProperties.logFns_oPlaylistHistory) {
			console.log("WSH_GBrowser: called oPlaylistHistory.trace ( )");
		}
	...
	this.saveCurrent = function () {
		if (globalProperties.logFns_oPlaylistHistory) {
			console.log("WSH_GBrowser: called oPlaylistHistory.saveCurrent ( )");
		}
		if (g_avoid_history) {
	...
	this.fullLibrary = function () {
		if (globalProperties.logFns_oPlaylistHistory) {
			console.log("WSH_GBrowser: called oPlaylistHistory.fullLibrary ( )");
		}
	...
	this.reset = function () {
		if (globalProperties.logFns_oPlaylistHistory) {
			console.log("WSH_GBrowser: called oPlaylistHistory.reset ( )");
		}
		this.history = Array();
	};
};
oFilterBox = function () {
	...
	this.getImages = function () {
		...
		this.search_bt = new button(
			...
			this.images.search_icon,
			"search_bt",
			"Filter groups" + (properties.filterBox_filter_tracks ? " & tracks" : "")
		);
...
function oTimers() {
	...
	this.brw_populate = function (callID, force_sorting, keep_showlist) {
		if (globalProperties.logFns_Timers) {
			console.log(`WSH_GBrowser: called oTimers.brw_populate (${callID}, ${force_sorting}, ${keep_showlist})`);
		}
...
oPlaylistManager = function (parentObjName) {
	...
	this.setPlaylistList = function () {
		if (globalProperties.logFns_oPlaylistManager) {
			console.log("WSH_GBrowser: called oPlaylistManager.setPlaylistList ( )");
		}
	...
	this.draw = function (gr) {
		if (globalProperties.logFns_oPlaylistManager) {
			//	console.log("WSH_GBrowser: called oPlaylistManager.draw ( )");
		}
		...
		if (globalProperties.drawDebugRects) {
			gr.DrawRect(brw.x, 0, brw.w, wh, 2, RGB(255, 0, 0));
		}
		...
		if ((g_dragA || g_dragR) && this.ishoverHeader) {
			...
			if (globalProperties.drawDebugRects) {
				gr.DrawRect(this.x + 1, this.y + 6, this.w - 2, g_fsize * 3, 2, RGB(255, 0, 0));
			}
		} else if (g_dragup_flash && g_flash_idx == -99) {
			gr.FillSolidRect(this.x + 1, this.y, this.w - 2, g_fsize * 3, colors.pm_hover_row_bg);
			if (globalProperties.drawDebugRects) {
				gr.DrawRect(this.x + 1, this.y, this.w - 2, g_fsize * 3, 2, RGB(255, 0, 0));
			}
		}
	...
	this.checkstate = function (event, x, y, delta) {
		if (globalProperties.logFns_oPlaylistManager) {
			//	console.log("WSH_GBrowser: called oPlaylistManager.checkstate ( )");
		}
...
oRow = function (metadb, itemIndex) {
	...
	this.getTags = function () {
		...
		if (properties.show2linesCustomTag != "") {
			...
			if (properties.showCodec) {
				if (properties.showBitrate) var TagsString = TF.titlePCB.EvalWithMetadb(metadb);
				else var TagsString = TF.titlePC.EvalWithMetadb(metadb);
			} else if (properties.showBitrate) var TagsString = TF.titlePB.EvalWithMetadb(metadb);
			else var TagsString = TF.titleP.EvalWithMetadb(metadb);
		} else if (properties.showCodec) {
			if (properties.showBitrate) var TagsString = TF.titleCB.EvalWithMetadb(metadb);
	...
	this.draw = function (gr, x, y, w) {
		...
		if (
			properties.showArtistName ||
			(properties.TFgrouping != "" && properties.TFgrouping.indexOf("artist%") == -1) ||
		...
		if (typeof brw.max_duration_length == "undefined" || brw.max_duration_length == 0)
			brw.max_duration_length = gr.CalcTextWidth("00:00:00", g_font.normal);
		...
		if (isPlaying && !properties.AlbumArtProgressbar && properties.drawProgressBar)
		...
		if (
			isPlaying &&
			properties.drawProgressBar &&
			!properties.AlbumArtProgressbar &&
		...
		if (
			...
			((properties.showRatingSelected && this.isSelected) ||
				(properties.showRatingRated && this.rating > 0) ||
				(!properties.showRatingSelected && !properties.showRatingRated))
		) {
			rating_vpadding = 4;
			if (typeof this.rating_length == "undefined" || this.rating_length == 0)
				this.rating_length = gr.CalcTextWidth("HHHHH", g_font.plus6);
			if (!g_showlist.ratingImages) {
				g_showlist.SetRatingImages(
					this.rating_length,
					text_height - rating_vpadding * (properties.show2lines ? 1 : 2),
		...
		if (this.tracknumber_w == 0)
			this.tracknumber_w = gr.CalcTextWidth(this.discnumber + this.tracknumber, g_font.normal) + 22;
		if (!isPlaying)
			gr.GdiDrawText(
		...
		if (
			...
			if (
				((properties.showPlaycount || properties.showCodec || properties.showBitrate) &&
	...
	this.check = function (event, x, y) {
		...
		this.ishover_rating =
			...
			(!properties.showRatingSelected || this.isSelected || (properties.showRatingRated && this.rating > 0));
				...
				if (this.ishover && y > brw.y) {
					...
					if (utils.IsKeyPressed(VK_SHIFT) && focusTrackIndex > -1) {
						...
						if (
							...
							properties.right_panel_follow_cursor
						)
							window.NotifyOthers(
								"trigger_on_focus_change",
								Array(brw.getSourcePlaylist(), playlistTrackId, this.metadb)
							);
					}
					brw.repaint();
				...
				if (this.ishover_rating && !g_dragR) {
					...
					if (properties.showToolTip && this.showToolTip && !(g_dragA || g_dragR || g_scrollbar.cursorDrag)) {
						if (this.ishover && brw.TooltipRow != this.itemIndex && !this.ishover_rating) {
							...
							g_tooltip.ActivateDelay(
								new_tooltip_text,
								x + 10,
								y + 20,
								globalProperties.tooltip_delay,
								1200
							);
						}
...
oShowList = function (parentPanelName) {
	...
	this.getColorSchemeFromImage = function () {
		...
		if (properties.circleMode && !properties.CoverGridNoText) image = brw.groups[this.idx].cover_img;
		...
		if (properties.AlbumArtProgressbar && this.light_bg) {
			...
			new_L < 10 &&
			(properties.showListColoredOneColor || properties.showListColoredMixedColor) &&
			properties.showListColored
		) {
	...
	this.getHeaderInfos = function (EvalWithMetadb) {
		if (globalProperties.logFns_oShowList) {
			console.log(`WSH_GBrowser: called oShowList.getHeaderInfos (${EvalWithMetadb})`);
		}
	...
	this.setColors = function () {
		if (globalProperties.logFns_oShowList) {
			console.log("WSH_GBrowser: called oShowList.setColors ( )");
		}
		...
		if (
			!(
				(properties.showListColoredOneColor ||
					properties.showListColoredMixedColor ||
					properties.showListColoredBlurred) &&
				properties.showListColored
			)
		) {
	...
	this.setSize = function () {
		if (globalProperties.logFns_oShowList) {
			//console.log("WSH_GBrowser: called oShowList.setSize ( )");
		}
		if (this.idx > -1) {
	...
	this.close = function () {
		if (globalProperties.logFns_oShowList) {
			//console.log("WSH_GBrowser: called oShowList.close ( )");
		}
		this.drawn_idx = -1;
		this.idx = -1;
		this.h = 0;
	...
	this.reset = function (idx, rowIdx, update_static_infos) {
		if (globalProperties.logFns_oShowList) {
			console.log(`WSH_GBrowser: called oShowList.reset (${idx}, ${rowIdx}, ${update_static_infos})`);
		}
	...
	this.CheckIfPlaying = function () {
		if (globalProperties.logFns_oShowList) {
			console.log("WSH_GBrowser: called oShowList.CheckIfPlaying ( )");
		}
	...
	this.haveSelectedRows = function () {
		if (globalProperties.logFns_oShowList) {
			console.log("WSH_GBrowser: called oShowList.haveSelectedRows ( )");
		}
	...
	this.getFirstSelectedRow = function () {
		if (globalProperties.logFns_oShowList) {
			console.log("WSH_GBrowser: called oShowList.getFirstSelectedRow ( )");
		}
		for (var i = 0; i < this.rows_.length; i++) {
			if (this.rows_[i].isSelected) {
				return this.rows_[i];
	...
	this.selectAll = function () {
		if (globalProperties.logFns_oShowList) {
			console.log("WSH_GBrowser: called oShowList.selectAll ( )");
		}
		var listIndex = [];
	...
	this.clearSelection = function () {
		if (globalProperties.logFns_oShowList) {
			console.log("WSH_GBrowser: called oShowList.clearSelection ( )");
		}
	...
	this.resetSelection = function () {
		if (globalProperties.logFns_oShowList) {
			console.log("WSH_GBrowser: called oShowList.resetSelection ( )");
		}
		for (var i = 0; i < this.rows_.length; i++) {
			this.rows_[i].isSelected = false;
		}
	...
	this.removeSelectedItems = function () {
		if (globalProperties.logFns_oShowList) {
			console.log("WSH_GBrowser: called oShowList.removeSelectedItems ( )");
		}
	...
	this.setMarginRight = function () {
		if (globalProperties.logFns_oShowList) {
			//console.log("WSH_GBrowser: called oShowList.setMarginRight ( )");
		}
		if (
	...
	this.saveCurrent = function () {
		if (globalProperties.logFns_oShowList) {
			console.log("WSH_GBrowser: called oShowList.saveCurrent ( )");
		}
	...
	this.restore = function () {
		if (globalProperties.logFns_oShowList) {
			console.log("WSH_GBrowser: called oShowList.restore ( )");
		}
	...
	this.refresh = function () {
		if (globalProperties.logFns_oShowList) {
			console.log("WSH_GBrowser: called oShowList.refresh ( )");
		}
		this.on_init();
		if (this.idx >= 0) {
	...
	this.refreshRows = function () {
		if (globalProperties.logFns_oShowList) {
			console.log("WSH_GBrowser: called oShowList.refreshRows ( )");
		}
		for (var i = this.rows_.length; i--; ) {
	...
	this.calcHeight = function (playlist, drawn_idx, columnsOffset, update_tracks, send_albums_info) {
		...
		if (properties.showlistScrollbar) this.heightMax = window.Height - brw.rowHeight - 100;
		else this.heightMax = 100000;

		if (this.heightMin > this.heightMax) this.heightMax = this.heightMin;
		if (this.totalColsVisMax > properties.showlistMaxColumns && properties.showlistMaxColumns > 0)
			this.totalColsVisMax = properties.showlistMaxColumns;

		if (properties.showlistOneColumn) this.totalColsVisMax = 1;
		...
		if (this.CoverSize > this.h - this.marginCover - 10)
			this.CoverSize = this.h - (properties.CoverGridNoText ? 0 : this.marginCover) - 10;
		...
		if (properties.showlistMaxColumns < this.totalColsVis && properties.showlistMaxColumns > 0)
			this.totalColsVis = properties.showlistMaxColumns;
	...
	this.setColumns = function (columnsOffset) {
		if (globalProperties.logFns_oShowList) {
			console.log(`WSH_GBrowser: called oShowList.setColumns (${columnsOffset})`);
		}
	...
	this.isHover_hscrollbar = function (x, y) {
		if (globalProperties.logFns_oShowList) {
			console.log("WSH_GBrowser: called oShowList.isHover_hscrollbar ( )");
		}
		if (!this.hscr_visible) {
	...
	this.setColumnsOffset = function (offset_value) {
		if (globalProperties.logFns_oShowList) {
			console.log(`WSH_GBrowser: called oShowList.setColumnsOffset (${offset_value})`);
		}
	...
	this.draw = function (gr) {
		if (globalProperties.logFns_oShowList) {
			//	console.log("WSH_GBrowser: called oShowList.draw ( )");
		}
		...
		if (
			(properties.showListColoredMixedColor || properties.showListColoredOneColor) &&
			properties.showListColored &&
		...
		if (this.delta > 0) {
			this.y = Math.round(
				eval(this.parentPanelName + ".y") +
					(this.rowIdx + 1) * eval(this.parentPanelName + ".rowHeight") +
					eval(this.parentPanelName + ".marginTop") -
					scroll_
			);
			if (
				...
				if (globalProperties.drawDebugRects) {
					gr.DrawRect(this.x, this.y + this.marginTop, this.w + g_scrollbar.w, slh + 1, 2, RGB(0, 255, 0));
				}
				...
				if (slh > 0) {
					...
					var arrow_x =
						brw.marginLR + arrowItemIdx * brw.thumbnailWidth - Math.round(brw.thumbnailWidth / 2) - 13;
				...
				if (ty < this.y + slh) {
					...
					if (this.timeTextLenght == 0)
						this.timeTextLenght = gr.CalcTextWidth(this.length + ",  " + this.total_tracks, g_font.normal);
					...
					if (typeof this.firstRowLength == "undefined")
						this.firstRowLength = gr.CalcTextWidth(this.firstRow, g_font.italicplus5);
					if (typeof this.secondRowLength == "undefined")
						this.secondRowLength = gr.CalcTextWidth(this.secondRow, g_font.plus2);
...
oHeaderBar = function (name) {
	...
	this.setSize = function (x, y, w, h) {
		if (globalProperties.logFns_oHeaderBar) {
			console.log(`WSH_GBrowser: called oHeaderBar.setSize (${x}, ${y}, ${w}, ${h})`);
		}
		//console.log(`WSH_GBrowser: called oHeaderBar.setSize ( )`);
		this.x = x;
		this.y = y;
		this.w = ww;
	...
	this.setHideButton = function () {
		if (globalProperties.logFns_oHeaderBar) {
			console.log("WSH_GBrowser: called oHeaderBar.setHideButton ( )");
		}
		this.hscr_btn_w = 18;
	...
	this.setButtons = function () {
		if (globalProperties.logFns_oHeaderBar) {
			console.log("WSH_GBrowser: called oHeaderBar.setButtons ( )");
		}
		var gb;
	...
	this.onColorsChanged = function () {
		if (globalProperties.logFns_oHeaderBar) {
			console.log("WSH_GBrowser: called oHeaderBar.onColorsChanged ( )");
		}
		this.setButtons();
		this.setHideButton();
	};
	this.draw = function (gr) {
		...
		if (globalProperties.drawDebugRects) {
			gr.DrawRect(0, 0, ww, this.h, 2, RGB(0, 128, 255));
		}

		//bottom line
		gr.FillSolidRect(0, this.h, ww, 1, colors.headerbar_line);
		if (globalProperties.drawDebugRects) {
			gr.DrawRect(0, this.h, ww, 1, 2, RGB(0, 128, 255));
		}
		...
		if (properties.showCoverResizer) {
			brw.drawResizeButton(
				gr,
				ww - this.rightpadding - 5 - this.MarginRight - gridmode_width,
				Math.round((this.h - brw.resize_bt.img[0].Height) / 2)
			);
		...
		if (!brw.showFilterBox) {
			this.mainTxtX = this.MarginLeft + this.btn_left_margin;
			this.mainTxtSpace =
				ww - this.resize_bt_w - this.rightpadding - this.RightTextLength - this.MarginRight - this.mainTxtX;
		...
		if (covers_loading_progress < 101 && properties.show_covers_progress)
			gr.GdiDrawText(
				...
				this.mainTxtX,
				0,
				ww + 60 - this.resize_bt_w - this.rightpadding - this.MarginRight - this.mainTxtX - gridmode_width,
				this.h,
	...
	this.isHover_Settings = function (x, y) {
		if (x > this.MarginLeft - 7 && x < this.MarginLeft + 23 && y > this.padding_top && y < this.padding_top + 23)
			return true;
		return false;
	};
	this.setDisplayedInfo = function () {
		if (globalProperties.logFns_oHeaderBar) {
			console.log("WSH_GBrowser: called oHeaderBar.setDisplayedInfo ( )");
		}
		this.timeTxt = "";
	...
	this.on_mouse = function (event, x, y) {
		if (globalProperties.logFns_oHeaderBar) {
			//	console.log("WSH_GBrowser: called oHeaderBar.on_mouse ( )");
		}
		this.ishover = x >= this.x && x <= this.x + this.w && y >= this.y && y <= this.y + this.h;
		...
		switch (event) {
			...
			case "lbtn_dblclk":
				if (
					brw.playlistName != globalProperties.whole_library &&
			...
			case "move":
				...
				if (typeof this.GridModeButton !== "undefined" && properties.showGridModeButton)
	...
	this.append_sort_menu = function (basemenu, actions) {
		if (globalProperties.logFns_oHeaderBar) {
			console.log(`WSH_GBrowser: called oHeaderBar.append_sort_menu (${basemenu}, actions)`);
		}
	...
	this.append_group_menu = function (basemenu, actions) {
		if (globalProperties.logFns_oHeaderBar) {
			console.log(`WSH_GBrowser: called oHeaderBar.append_group_menu (${basemenu}, actions)`);
		}
...
function draw_settings_menu(x, y, right_align, sort_group) {
	if (globalProperties.logFns_settings_menu) {
		console.log(`WSH_GBrowser: called draw_settings_menu (${x}, ${y}, ${right_align}, ${sort_group})`);
	}
	...
	_menuGroupDisplay.CheckMenuRadioItem(
		100,
		102,
		properties.CoverGridNoText ? 102 : properties.circleMode ? 101 : 100
	);
	...
	_menuGroupDisplay.AppendMenuItem(
		properties.CoverGridNoText | properties.circleMode ? MF_GRAYED : MF_STRING,
		38,
		"Center text"
	);
	...
	_menuCoverShadow.AppendMenuItem(
		MF_STRING,
		48,
		"Set shadow opacity (current:" + properties.default_CoverShadowOpacity + ")"
	);
	...
	if (properties.show2linesCustomTag != "") _additionalInfos.AppendMenuItem(MF_STRING, 62, "Reset");
	...
	_menuProgressBar.CheckMenuRadioItem(
		21,
		24,
		!properties.drawProgressBar ? 21 : properties.AlbumArtProgressbar ? 23 : 24
	);
	...
	_menuRating.CheckMenuRadioItem(
		32,
		36,
		properties.showRating && !properties.showRatingSelected && !properties.showRatingRated
			? 32
			: properties.showRating && properties.showRatingSelected && !properties.showRatingRated
			? 33
			: properties.showRating && properties.showRatingSelected && properties.showRatingRated
			? 34
			: properties.showRating && properties.showRatingRated
			? 36
			: 35
	);
	...
	switch (true) {
		...
		case idx == 13:
			properties.smooth_expand_value =
				properties.smooth_expand_value > 0 ? 0 : properties.smooth_expand_default_value;
		...
		case idx == 51:
			...
			window.SetProperty(
				"MAINPANEL adapt now playing to left menu righ playlist on",
				properties.showInLibrary_RightPlaylistOn
			);
		...
		case idx == 53:
			...
			window.SetProperty(
				"MAINPANEL adapt now playing to left menu righ playlist off",
				properties.showInLibrary_RightPlaylistOff
			);
			window.NotifyOthers("showInLibrary_RightPlaylistOn", properties.showInLibrary_RightPlaylistOn);
			setShowInLibrary();
			break;
		...
		case idx == 48:
			try {
				new_value = utils.InputBox(
					...
					properties.default_CoverShadowOpacity,
					true
				);
		...
		case idx == 328:
			...
			window.SetProperty(
				"MAINPANEL Automatically change displayed playlist",
				properties.enableAutoSwitchPlaylistMode
			);
			brw.populate(0);
			break;
		case idx == 329:
...
oScrollbar = function (parentObjectName) {
	if (globalProperties.logFns_oScrollbar) {
		console.log(`WSH_GBrowser: called oScrollbar (${parentObjectName})`);
	}
	...
	this.draw = function (gr, x, y) {
		if (globalProperties.logFns_oScrollbar) {
			//	console.log("WSH_GBrowser: called oScrollbar.draw ( )");
		}
	...
	this.get_h_tot = function () {
		if (globalProperties.logFns_oScrollbar) {
			//console.log("WSH_GBrowser: called oScrollbar.get_h_tot ( )");
		}
		if (g_showlist.idx > -1) {
			if ((window.Height - brw.headerBarHeight) % brw.rowHeight < colors.fading_bottom_height * 0.5) {
			...
			if ((window.Height - brw.headerBarHeight) % brw.rowHeight < colors.fading_bottom_height * 0.6) {
	...
	this.check_scroll = function (scroll_to_check) {
		if (globalProperties.logFns_oScrollbar) {
			//console.log("WSH_GBrowser: called oShowList.check_scroll ( )");
		}
	...
	this.setButtons = function () {
		if (globalProperties.logFns_oScrollbar) {
			//console.log("WSH_GBrowser: called oScrollbar.setButtons ( )");
		}
	...
	this.setSize = function (x, y, whover, h, wnormal) {
		if (globalProperties.logFns_oScrollbar) {
			//console.log("WSH_GBrowser: called oScrollbar.setSize ( )");
		}
		this.x = x;
	...
	this.cursorCheck = function (event, x, y) {
		if (globalProperties.logFns_oScrollbar) {
			//	console.log("WSH_GBrowser: called oScrollbar.cursorCheck ( )");
		}
		this.ishover =
	...
	this.check = function (event, x, y) {
		if (globalProperties.logFns_oScrollbar) {
			//	console.log("WSH_GBrowser: called oScrollbar.check ( )");
		}
	...
	this.repaint = function () {
		if (globalProperties.logFns_oScrollbar) {
			//	console.log("WSH_GBrowser: called oScrollbar.repaint ( )");
		}
...
oBrowser = function (name) {
	...
	if (properties.showheaderbar) this.headerBarHeight = properties.CoverGridNoText ? 39 : 43;
	else this.headerBarHeight = properties.CoverGridNoText ? 0 : 4;
	...
	this.FormatTime = function (time) {
		if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.FormatTime (${time})`);
		}
		time_txt = "";
		if (time > 0) {
	...
	this.showheaderbar = function () {
			if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.showheaderbar ( )`);
		}
		if (properties.showheaderbar) this.headerBarHeight = properties.CoverGridNoText ? 39 : 43;
		else this.headerBarHeight = properties.CoverGridNoText ? 0 : 4;
	...
	this.toggle_grid_mode = function (circleMode, gridMode) {
			if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.toggle_grid_mode (${circleMode}, ${gridMode})`);
		}
	...
	this.switch_display_mode = function () {
			if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.switch_display_mode ( )`);
		}
	...
	this.on_font_changed = function (refreshDates) {
			if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.on_font_changed (${refreshDates})`);
		}
	...
	this.on_init = function () {
		if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.on_init ( )`);
		}
		if (properties.CoverGridNoText) {
	...
	this.get_albums = function (start, str_comp) {
		...
		while (k < this.totalTracks) {
			...
			if (string_compare != temp) {
				...
				if (properties.extractYearFromDate && properties.showdateOverCover)
	...
	this.refreshDates = function () {
			if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.refreshDates ( )`);
		}
	...
	this.getPlaybackPlaylist = function () {
			if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.getPlaybackPlaylist ( )`);
		}
	...
	this.getSelectionPlaylist = function () {
		if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.getSelectionPlaylist ( )`);
		}
		g_avoid_on_playlists_changed = true;
	...
	this.getWholeLibraryPlaylist = function () {
			if (globalProperties.logFns_oBrowser) {
			    console.log(`WSH_GBrowser: called oBrowser.getWholeLibraryPlaylist ( )`);
		    }
		g_avoid_on_playlists_changed = true;
		...
		if (!isWholeLibraryPlaylistFound) {
			plman.CreateAutoPlaylist(
				...
				"ALL ",
				"%artist% | %album% | $if(%album%,%date%,'9999') | %tracknumber% | %title%",
				1
			);
			plman.MovePlaylist(total, 0);
	...
	this.setSourcePlaylist = function () {
		if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.setSourcePlaylist ( )`);
		}
		this.SourcePlaylistIdx = this.calculateSourcePlaylist();
	};
	this.calculateSourcePlaylist = function () {
			if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.calculateSourcePlaylist ( )`);
		}
	...
	this.getSourcePlaylist = function () {
		if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.getSourcePlaylist ( )`);
		}
		return this.SourcePlaylistIdx;
	};
	...
	this.sortAccordingToProperties = function (force_sorting) {
	    if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.sortAccordingToProperties (${force_sorting})`);
		}
		if (
			(properties.TFsorting != properties.TFsorting_default && force_sorting) ||
			properties.TFsorting_default == ""
		) {
	...
	this.refresh_browser_images = function () {
        if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.refresh_browser_images ( )`);
		}
		this.coverMask = false;
		this.dateCircleBG = false;
		for (var i = 0; i < this.groups.length; i++) {
	...
	this.refresh_browser_thumbnails = function () {
        if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.refresh_browser_thumbnails ( )`);
		}
		this.coverMask = false;
		this.dateCircleBG = false;
		for (var i = 0; i < this.groups.length; i++) {
	...
	this.refresh_shadows = function () {
	        if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.refresh_shadows ( )`);
		}
	...
	this.refresh_one_image = function (albumIndex) {
		        if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.refresh_one_image ( )`);
		}
	...
	this.refresh_all_images = function () {
			        if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.refresh_all_images ( )`);
		}
		this.coverMask = false;
		this.dateCircleBG = false;
		for (var i = 0; i < this.groups.length; i++) {
	...
	this.freeMemory = function () {
				        if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.freeMemory ( )`);
		}
		this.refresh_all_images();
	};
	this.GetFilteredTracks = function (idx) {
		 if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.GetFilteredTracks ( )`);
		}
	...
	this.GetAlbumCover = function (idx) {
			 if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.GetAlbumCover (${idx})`);
		}
		var img_thumb = null;
		var img_full = null;
		...
		if (isImage(this.groups[idx].cover_img)) {
			img_thumb = FormatCover(
				...
				this.coverRealWith + (properties.CoverGridNoText ? 2 : 0),
				this.coverRealWith + (properties.CoverGridNoText ? 2 : 0),
				false,
				"GetAlbumCover1"
			);
	...
	this.draw = function (gr) {
		...
		if (repaint_main || repaint_f || !repaintforced) {
			...
			if (g_showlist.idx > -1) {
					...
					g_showlist.delta_ < g_showlist.delta * this.rowHeight
						? g_showlist.delta_ +
								(g_showlist.delta * this.rowHeight - g_showlist.delta_) *
									(1 - properties.smooth_expand_value)
				...
				g_showlist.delta_ = Math.ceil(
					g_showlist.delta_ > 5
						? g_showlist.delta_ - g_showlist.delta_ * (1 - properties.smooth_expand_value)
						: 0
				);
				...
				if (ay >= 0 - this.rowHeight && ay < this.y + this.h) {
					...
					if (
						...
						if (properties.showCoverShadow && properties.CoverShadowOpacity > 0) {
							if (!this.cover_shadow || this.cover_shadow == null)
								this.cover_shadow = createCoverShadowStack(
									...
									colors.cover_shadow,
									10,
									properties.circleMode && !properties.CoverGridNoText
								);
						...
						if (properties.showdateOverCover && this.groups[this.groups_draw[i]].date != "?") {
							if (properties.extractYearFromDate)
						...
						if (
							(!properties.expandInPlace || this.groups_draw.length == 1) &&
						...
						try {
							...
							if (
								...
								gr.GdiDrawText(
									...
									this.coverRealWith,
									50 + g_fsize,
									(properties.centerText | properties.circleMode ? DT_CENTER : DT_LEFT) |
										DT_TOP |
										DT_END_ELLIPSIS |
										DT_NOPREFIX
								);
			...
			if (g_resizing.isResizing()) {
				...
				if (globalProperties.drawDebugRects) {
					gr.DrawRect(ww - 1, 0, 1, wh, 2, RGB(0, 255, 0));
				} else gr.FillSolidRect(0, 0, 1, wh, colors.dragdrop_marker_line);
				if (globalProperties.drawDebugRects) {
					gr.DrawRect(0, 0, 1, wh, 2, RGB(0, 255, 0));
				}
			}
	...
	this.stopResizing = function () {
		if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.stopResizing ( )`);
		}
	...
	this.stopDragging = function (x, y) {
		if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.stopDragging (${x}, ${y})`);
		}
		if (g_dragA) {
			g_dragA_idx = -1;
			g_dragA = false;
	...
	this.playGroup = function (group_id) {
			if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.playGroup (${group_id})`);
		}
	...
	this.on_mouse = function (event, x, y) {
		if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.on_mouse (${event}, ${x}, ${y})`);
		}
		this.ishover = x >= this.x && x <= this.x + this.w && y >= this.y && y <= this.y + this.h;
		//console.log(`this.h: ${this.h} this.w: ${this.w} this.x: ${this.x} this.y: ${this.y}`);
		switch (event) {
	...
	this.timerScript = function () {
		...
		if (
			properties.showheaderbar &&
			brw.finishLoading &&
			properties.show_covers_progress &&
	...
	this.drawResizeButton = function (gr, x, y) {
		...
		resizeCursorPos = Math.round(
			(this.resize_bt.w * (properties.thumbnailWidth - properties.thumbnailWidthMin)) /
				(this.thumbnailWidthMax - properties.thumbnailWidthMin)
		);
		...
		if (globalProperties.drawDebugRects) {
			gr.DrawRect(x + resizeCursorPos, y + Math.round(this.resize_bt.h / 2) - 6, 1, 10, 2, RGB(0, 255, 0));
		} else gr.FillSolidRect(x + resizeCursorPos, y + Math.round(this.resize_bt.h / 2) - 6, 1, 10, colors.faded_txt);
		if (globalProperties.drawDebugRects) {
			gr.DrawRect(x + resizeCursorPos, y + Math.round(this.resize_bt.h / 2) - 6, 1, 10, 2, RGB(0, 255, 0));
		}
	};
	this.moveResizeBtn = function (x, y) {
		...
		properties.thumbnailWidth = Math.round(
			(this.thumbnailWidthMax - properties.thumbnailWidthMin) * new_value + properties.thumbnailWidthMin
		);
		if (properties.thumbnailWidth > this.thumbnailWidthMax) properties.thumbnailWidth = this.thumbnailWidthMax;
		else if (properties.thumbnailWidth < properties.thumbnailWidthMin)
	...
	this.seek_track = function (metadb, albumIdx) {
		if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.seek_track (${metadb}, ${albumIdx})`);
		}
	...
	this.focus_on_nowplaying = function (track) {
		if (globalProperties.logFns_oBrowser) {
			console.log(`WSH_GBrowser: called oBrowser.focus_on_nowplaying (${track})`);
		}
		FocusOnNowPlaying = true;
		if (!track) return;
...
function toggleLibraryFilterState() {
	if (globalProperties.logFns_oPanelSetting) {
		console.log(`WSH_GBrowser: called toggleLibraryFilterState ( )`);
	}
	libraryfilter_state.toggleValue();
	if (libraryfilter_state.isActive()) {
		properties.showFilterBox = properties.showFilterBox_filter_active;
			...
			case "move":
				if (this.tooltip_text != "" && g_tooltip.activeZone != this.text) {
					g_tooltip.ActivateDelay(
						...
						x + 10,
						y + 20,
						globalProperties.tooltip_button_delay,
						1200,
						false,
						this.text
					);
					...
					g_tooltip.activeZone == this.text
				) {
					this.tooltip_activated = false;
					g_tooltip.Deactivate();
				}
...
function populate_with_library_covers(start_items, str_comp_items) {
	if (globalProperties.logFns_oImageCache) {
		console.log(`WSH_GBrowser: called populate_with_library_covers (${start_items}, ${str_comp_items})`);
	}
	if (start_items == 0) {
	...
	while (covers_current_item < total) {
		...
		if (string_compare_items != string_current_item) {
			...
			if (globalProperties.load_artist_img_at_startup)
				cachekey_artist = process_cachekey(
					covers_FullLibraryList[covers_current_item],
					properties.tf_crc_artist
				);
			if (globalProperties.enableDiskCache) {
				...
				if (globalProperties.load_artist_img_at_startup && cachekey_artist != "undefined") {
					current_item_filename_artist = check_cache(
						covers_FullLibraryList[covers_current_item],
						0,
						cachekey_artist
					);
					if (current_item_filename_artist) {
						g_image_cache.addToCache(
							load_image_from_cache_direct(current_item_filename_artist),
							cachekey_artist
						);
					}
				}
...
function toggleBlurWallpaper(wallpaper_blur_state) {
	wallpaper_blur_state =
		typeof wallpaper_blur_state !== "undefined" ? wallpaper_blur_state : !properties.wallpaperblurred;
...
function on_size(w, h) {
	...
	if (window.IsVisible || first_on_size) {
		...
		brw.setSize(0, brw.headerBarHeight, ww, wh - brw.headerBarHeight);
		g_scrollbar.setSize(
			ww - cScrollBar.activeWidth,
			brw.y - brw.headerBarHeight,
			cScrollBar.activeWidth,
			wh - brw.y + brw.headerBarHeight,
...
function set_update_function(string) {
	if (globalProperties.logFns_Misc) {
		console.log(`WSH_GBrowser: called set_update_function (${string})`);
	}
...
function on_paint(gr) {
	//if (repaint_main) repaintRects.push({ x:5, y:5, w:ww-5, h:wh-5 });
	...
	if (globalProperties.drawDebugRects) {
		gr.DrawRect(0, 0, ww, wh, 2, RGB(255, 128, 0));
	}
	if (g_wallpaperImg && properties.showwallpaper) {
		...
		gr.FillSolidRect(
			...
			ww,
			wh,
			properties.wallpaperblurred ? colors.wallpaper_overlay_blurred : colors.wallpaper_overlay
		);
		if (globalProperties.drawDebugRects) {
			gr.DrawRect(0, 0, ww, wh, 2, RGB(0, 255, 0));
		}
	}
	...
	gr.FillGradRect(
		0,
		wh - colors.fading_bottom_height,
		ww,
	...
	if (globalProperties.drawDebugRects) {
		gr.DrawRect(0, wh - colors.fading_bottom_height, ww, colors.fading_bottom_height, 2, RGB(0, 255, 0));
	}

	if (!properties.showheaderbar && !properties.CoverGridNoText)
		gr.FillSolidRect(0, 0, ww - 1, brw.marginTop + brw.headerBarHeight + 4, colors.no_headerbar_top);
	if (globalProperties.drawDebugRects) {
		gr.DrawRect(0, 0, ww - 1, brw.marginTop + brw.headerBarHeight + 4, 2, RGB(255, 128, 0));
	}
	...
	if (repaintRects.length) {
		repaintRects.forEach((rect) => gr.DrawRect(rect.x, rect.y, rect.w, rect.h, 2, RGB(255, 0, 0)));
		repaintRects = [];
	}
}
...
function on_mouse_lbtn_down(x, y, m) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_mouse_lbtn_down (${x}, ${y}, ${m})`);
	}
	var isResizing = g_resizing.on_mouse(
		"lbtn_down",
		x,
		y,
		m,
		!g_scrollbar.ishover &&
			((g_showlist.idx > -1 &&
				g_showlist.prev_bt.state != ButtonStates.hover &&
				g_showlist.next_bt.state != ButtonStates.hover) ||
				g_showlist.idx <= -1)
	);
	if (!isResizing) {
...
function on_mouse_lbtn_up_delayed(x, y) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_mouse_lbtn_up_delayed (${x}, ${y})`);
	}
...
function on_mouse_lbtn_up(x, y, m) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_mouse_lbtn_up (${x}, ${y}, ${m})`);
	}
...
function on_mouse_lbtn_dblclk(x, y, mask) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_mouse_lbtn_dblclk (${x}, ${y}, ${mask})`);
	}
	doubleClick = true;
...
function on_mouse_rbtn_down(x, y) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_mouse_rbtn_down (${x}, ${y})`);
	}
...
function on_mouse_wheel(step, stepstrait, delta) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_mouse_wheel (${delta})`);
	}
	...
	if (utils.IsKeyPressed(VK_CONTROL)) {
		...
		if (!timers.mouseWheel) {
			if (intern_step > 0) {
				...
				if (globalProperties.fontAdjustement > globalProperties.fontAdjustement_max)
					globalProperties.fontAdjustement = globalProperties.fontAdjustement_max;
			} else {
				globalProperties.fontAdjustement -= zoomStep;
				if (globalProperties.fontAdjustement < globalProperties.fontAdjustement_min)
					globalProperties.fontAdjustement = globalProperties.fontAdjustement_min;
			}
...
function on_mouse_leave() {
	if (globalProperties.logFns_Callbacks) {
		console.log("WSH_GBrowser: called on_mouse_leave ( )");
	}
...
function get_colors() {
	...
	if (properties.darklayout) {
		...
		properties.CoverShadowOpacity =
			(255 - properties.default_CoverShadowOpacity) * 0.2 + properties.default_CoverShadowOpacity;

		colors.cover_shadow = GetGrey(0, properties.CoverShadowOpacity);
		colors.cover_shadow_hover = GetGrey(
			0,
			((255 - properties.CoverShadowOpacity) * 2) / 3 + properties.CoverShadowOpacity
		);
		...
		colors.cover_shadow_hover = GetGrey(
			0,
			(255 - properties.CoverShadowOpacity) * 0.15 + properties.CoverShadowOpacity
		);
		colors.cover_shadow_bg = GetGrey(255);
...
function on_playback_stop(reason) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_playback_stop (${reason})`);
	}
	g_seconds = 0;
...
function on_playback_new_track(metadb) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_playback_new_track (${metadb})`);
	}
	g_showlist.CheckIfPlaying();
	g_seconds = 0;
	try {
	...
	if (window.IsVisible) {
		if (
			(properties.followNowPlaying && !getRightPlaylistState()) ||
			g_showlist.isPlaying ||
			(FocusOnNowPlaying && !brw.firstInitialisation)
		) {
			if (
				plman.ActivePlaylist != plman.PlayingPlaylist &&
				properties.followNowPlaying &&
				!getRightPlaylistState()
			) {
				plman.ActivePlaylist = plman.PlayingPlaylist;
			}
...
function on_playback_time(time) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_playback_time (${time})`);
	}
	g_seconds = time;
	if (window.IsVisible) {
...
function on_playback_seek(time) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_playback_seek (${time})`);
	}
	g_seconds = time;
	if (window.IsVisible) {
		if (g_showlist.idx > -1 && g_showlist.playing_row_w > 0) {
...
function on_playlist_switch() {
	if (globalProperties.logFns_Callbacks) {
		console.log("WSH_GBrowser: called on_playlist_switch ( )");
	}
	if (brw.followActivePlaylist) {
...
function on_playlist_items_reordered(playlist) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_playlist_items_reordered (${playlist})`);
	}
...
function on_playlists_changed() {
	if (globalProperties.logFns_Callbacks) {
		console.log("WSH_GBrowser: called on_playlists_changed ( )");
	}
...
function on_playlist_items_selection_change() {
	if (globalProperties.logFns_Callbacks) {
		console.log("WSH_GBrowser: called on_playlist_items_selection_change ( )");
	}
	if (window.IsVisible) brw.repaint();
...
function on_playlist_items_added(playlist) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_playlist_items_added (${playlist})`);
	}
	source_playlist_idx = brw.calculateSourcePlaylist();
	if (brw.followActivePlaylist || source_playlist_idx == playlist) {
...
function on_playlist_items_removed(playlist) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_playlist_items_removed (${playlist})`);
	}
	source_playlist_idx = brw.calculateSourcePlaylist();
	if (brw.followActivePlaylist || source_playlist_idx == playlist) {
...
function on_library_items_added() {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_library_items_added ( )`);
	}
	if (LibraryItems_counter < 1) {
...
function on_metadb_changed(metadbs, fromhook) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_metadb_changed (${metadbs}, ${fromhook})`);
	}
	if (window.IsVisible) {
...
function on_drag_enter(action, x, y, mask) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_drag_enter (${action}, ${x}, ${y}, ${mask})`);
	}
	action.Effect = 0;
}
...
function on_drag_leave() {
	if (globalProperties.logFns_Callbacks) {
		console.log("WSH_GBrowser: called on_drag_leave ( )");
	}
...
function on_drag_drop(action, x, y, mask) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_drag_drop (${action}, ${x}, ${y}, ${mask})`);
	}
	if (g_dragA || g_dragR) {
		g_resizing.on_mouse("lbtn_up", 0, 0, null);
	}
...
function on_drag_over(action, x, y, mask) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_drag_over (${action}, ${x}, ${y}, ${mask})`);
	}
	if (!(g_dragA || g_dragR)) {
		action.Effect = 0;
		return;
...
function on_notify_data(name, info) {
	if (globalProperties.logFns_Misc) {
		console.log(`WSH_GBrowser: called on_notify_data (${name}, ${info})`);
	}
	switch (name) {
...
function on_char(code) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_char (${code})`);
	}
	// inputBox
	if (brw.showFilterBox && properties.showheaderbar && g_filterbox.inputbox.visible) {
...
function on_key_up(vkey) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_key_up (${vkey})`);
	}
	// inputBox
	if (brw.showFilterBox && properties.showheaderbar && g_filterbox.inputbox.visible) {
...
function on_key_down(vkey) {
	if (globalProperties.logFns_Callbacks) {
		console.log(`WSH_GBrowser: called on_key_down (${vkey})`);
	}
...
function on_init() {
	...
	if (
		(globalProperties.load_covers_at_startup || globalProperties.load_artist_img_at_startup) &&
		globalProperties.enableDiskCache
	) {
